...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const [textLines, setTextLines] = useState<string[]>(['Contents', 'Close']);
  ...
  const animateText = useCallback((opening: boolean) => {
    ...
    const currentLabel = opening ? 'Contents' : '';
    const targetLabel = opening ? '' : 'Contents';
    const cycles = 3;

    const seq: string[] = [currentLabel];
    let last = currentLabel;
    for (let i = 0; i < cycles; i++) {
      last = last === 'Contents' ? '' : 'Contents';
      seq.push(last);
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          <button
            ref={toggleBtnRef}
            className={`sm-toggle relative mt-5 inline-flex items-center justify-center gap-[0.3rem] bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible pointer-events-auto ${
                ...
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    <a
                      ...
                      onClick={toggleMenu}
                    >
                      <span className="sm-panel-itemLabel text-4xl inline-block [transform-origin:50%_100%] will-change-transform">
                        {it.label}
                      </span>
                    </a>
                  </li>
